  ...
  () => {
    ...
    async function createBrandIfNotExists(brandPage: any, name: string) {
      ...
      if (isEmpty) {
        ...
        await expect(
          brandPage.toastMessage.success_message.first(),
        ).toBeVisible();
    ...
    test('TC026 - Update brand name successfully', async ({ brandPage }) => {
      ...
      await test.step('Update brand name', async () => {
        await brandPage.attributeModalComponent.updateAttribute(
          'Brand',
          newName,
        );
      });
    ...
    test('TC028 - Validate update with duplicate name', async ({
      brandPage,
    }) => {
    ...
    test('TC029 - Verify updated brand persists after reload', async ({
      brandPage,
    }) => {
      const oldName = faker.company.name();
      const newName = `Brand_${faker.string.alphanumeric(5)}`;
    ...
    test('TC031 - Verify brand status persists after reload', async ({
      brandPage,
    }) => {
      let oldStatus: string | null;
      let newStatus: string | null;
